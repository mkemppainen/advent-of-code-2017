...
404	1757	358	2293	2626	87	613	95	1658	147	75	930	2394	2349	86	385
  ...
  def min_max_difference([x|xs]) do
    case even_divide(x, xs) do
      x when is_number(x) -> x
      _ -> min_max_difference(xs)
    end
  end

  def even_divide(_, []), do: :not_divisible
  def even_divide(x, [y|_]) when rem(x,y) == 0, do: div(x,y)
  def even_divide(x, [y|_]) when rem(y,x) == 0, do: div(y,x)
  def even_divide(x, [_|ys]), do: even_divide(x, ys)
  
end

Day2.solve2
# Solution.min_max_difference([2,3,9])
|> IO.puts
